...
public class SmModuleManager implements ISmModuleManager {
    ...
    public SmModuleManager(ISmExtensionManager extensionManager) {
        ...
        // 初始化时注册内置扩展槽
        initializeBuiltinExtensionSlots();
    }
     ...
     * 初始化内置扩展槽
     */
    private void initializeBuiltinExtensionSlots() {
        try {
            // 注册内置扩展槽
            int registeredCount = SmSlotRegistrar.registerAllBuiltinSlots(extensionManager);
            if (registeredCount > 0) {
                log.info("Successfully initialized {} builtin extension slots", registeredCount);
            } else {
                log.warn("No builtin extension slots were registered");
            }
        } catch (Exception e) {
            log.error("Error initializing builtin extension slots", e);
        }
    }
}