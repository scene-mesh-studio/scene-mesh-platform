...
import com.scene.mesh.foundation.impl.module.scanner.ExtensionPluginScanner;
        ...
        try {
            ...
            if (manifest != null) {
                ...
                if (mainClass != null && !mainClass.trim().isEmpty()) {
                    try {
                        // 尝试加载 Main-Class 并检查是否实现了 ISmModule 接口
                        Class<?> mainClassObj = classLoader.loadClass(mainClass.trim());
                        Object instance = mainClassObj.getDeclaredConstructor().newInstance();
                        
                        if (instance instanceof SmModule) {
                            SmModule module = (SmModule) instance;
                            
                            // 添加 JAR 相关的元数据
                            java.util.Map<String, Object> metas = new java.util.HashMap<>(module.getMetas());
                            ...
                            metas.put("mainClass", mainClass);
                            metas.put("extensionPluginCount", module.getExtensionCapacities().size());
                            ...
                            log.warn("Main class {} does not extend SmModule class, falling back to default module creation", mainClass);
                        }
                    } catch (Exception e) {
                        log.warn("Failed to instantiate main class {}: {}, falling back to default module creation", mainClass, e.getMessage());
                    }
                }
            }
